...
import static com.lckiss.weekly.MainActivity.setCurrentItem;
...
public class LineView extends View {
    ...
    //自定义点击pop
    public int SHOW_POPUPS = 0;

    private int showPopupType = SHOW_POPUPS;
    ...
    public void setDataList(ArrayList<Float> dataLists) {
        setFloatDataList(dataLists, false);
    }

    public void setFloatDataList(ArrayList<Float> dataList, boolean showFloatNumInPopup) {
        selectedDot = null;
...
*/
    ...
    private int getVerticalGridlNum() {
        ...
        if (dataLists==null){
            Log.d("info", "LineView...getVerticalGridlNum occur exception");
        ...
        if (dataLists != null && !dataLists.isEmpty()) {
            ...
            for (int k = 0; k < drawDotLists.size(); k++) {
                ...
                for (int i = 0; i < dataLists.size(); i++) {
                    ...
                    if (i > drawDotSize - 1) {
                        drawDotLists.get(k).add(new Dot(x, 0, x, y, dataLists.get(i), k));
                    } else {
                        drawDotLists.get(k).set(i, drawDotLists.get(k).get(i).setTargetData(x, y, dataLists.get(i), k));
                    }
                }

                int temp = drawDotLists.get(k).size() - dataLists.size();
...
//        removeCallbacks(animator);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int k = 0; k < drawDotLists.size(); k++) {
            float maxValue = Collections.max(dataLists);
            float minValue = Collections.min(dataLists);
            int flag = 0;
    ...
    private void drawPopup(Canvas canvas, float num, Point point, int PopupColor) {
        ...
        NinePatchDrawable popup = (NinePatchDrawable) getResources().getDrawable(R.mipmap.popup_white);
