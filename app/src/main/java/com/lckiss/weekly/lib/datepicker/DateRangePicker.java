...
import com.lckiss.weekly.lib.common.util.DateUtils;
import com.lckiss.weekly.lib.common.util.LogUtils;
import com.lckiss.weekly.lib.datepicker.widget.WheelView;
...
public class DateRangePicker extends WheelPicker {
    ...
    protected View makeCenterView() {
        ...
        //范围选择起始标识
        if (isRange) {
            TextView startTips = new TextView(activity);
        ...
        if (!isRange){
            ...
            tips.setLayoutParams(new LinearLayout.LayoutParams(MATCH_PARENT, WRAP_CONTENT));
            tips.setGravity(Gravity.LEFT);
            tips.setText("选择日期:");
            tips.setTextSize(textSize);
            tips.setTextColor(textColorFocus);
            tips.setPadding(50, 0, 0, 20);
            root.addView(tips);
            root.setPadding(0,0,0,30);
        }
        root.addView(startLayout);
        if (isRange) {
            //范围选择结束标识
            TextView endTips = new TextView(activity);
